...
#if defined(NDEBUG)
# error "TelePayCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3b642408f7532914a5929e9f287b1f42907f10d94aa0ec3b6c8781fe37d5bd8b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TelePayCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TelePayCoin Signed Message:\n";
...
{
    // TelePayCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 571428); // TelePayCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // TelePayCoin: 2.5 days
static const int64 nTargetSpacing = 120; // TelePayCoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TelePayCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TelePayCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x175c2da75e9923a8b09d76876badefd59217bb8977e7a827213e986741f94b3c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "PAYMENTS SYSTEM MADES TELEPAY GLOBAL PAYEMENTS INC";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0426d515f16d3c8d1c33e3392d116acaf6e8667559dac56d646ff5312719a3f8bc7738ca072b4544971dd7d07c174d546a74bc8be59ac5e66e0e8496f95a1313e9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521594830;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085028961;

        if (fTestNet)
        {
            block.nTime    = 1521594816;
            block.nNonce   = 385299255;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9677f0cc2fa1eab8f36e64d5071dac5957e08217c42b441d0a88524ed10ed720"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xa0, 0xbc, 0xd1 }; // TelePayCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TelePayCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TelePayCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TelePayCoinMiner(CWallet *pwallet)
{
    printf("TelePayCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("telepaycoin-miner");
    ...
    try { loop {
        ...
        printf("Running TelePayCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TelePayCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TelePayCoinMiner, pwallet));
}
